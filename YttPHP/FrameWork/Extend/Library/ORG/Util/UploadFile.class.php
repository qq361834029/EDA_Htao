...
{//类定义开始
    ...
    // 允许上传的文件类型
    // 留空不做检查
    public $allowTypes = array('jpg','gif','bmp','png','jpeg');
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    private function save($file,$relation_type='')
    {
		...
		if($relation_type==11){
			if(!move_uploaded_file($file['tmp_name'], auto_charset($filename,'utf-8','iso-8859-1'))) {
				$this->error = '文件上传保存错误！';
				return false;
			}
		}else{
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    public function uploadOne($file,$savePath='',$relation_type=''){
        //如果不指定保存文件名，则由系统默认
        if(empty($savePath))
            $savePath = $this->savePath;
        // 检查上传目录
        ...
        if(!empty($file['name'])) {
            ...
            foreach ($fileArray as $key=>$file){
                ...
                $file['extension']  = $this->getExt($file['name']);
                $file['savepath']   = $savePath;
                $file['savename']   = $file['savename'] ? $file['savename'].'.'.$file['extension'] : $this->getSaveName($file).'.'.$file['extension'];
                // 自动检查附件
                if($this->autoCheck) {
                    if(!$this->check($file))
                        return false;
                ...
                //保存上传文件
                if(!$this->save($file,$relation_type)) return false;
                if(function_exists($this->hashType)) {
                    $fun =  $this->hashType;
                    $file['hash']   =  $fun(auto_charset($file['savepath'].$file['savename'],'utf-8','gbk'));
                }
