...
class Barcode {
    ...
    private static function digitToRenderer($res, $xi, $yi, $angle, $mw, $mh, $digit, $color = null){
        $lines = count($digit);
    ...
    private static function digitToGDRenderer($gd, $color, $xi, $yi, $angle, $mw, $mh, $digit){
        return self::digitToRenderer($gd, $xi, $yi, $angle, $mw, $mh, $digit,$color);
    }
    ...
    private static function digitToFPDFRenderer($pdf, $color, $xi, $yi, $angle, $mw, $mh, $digit){
        ...
        return self::digitToRenderer($pdf, $xi, $yi, $angle, $mw, $mh, $digit);
    }
	
	static private function fn($points, $res, $color=null){
		if (!is_null($color)) {
			imagefilledpolygon($res, $points, 4, $color);
		} else {
            $op = 'f';
            $h = $res->h;
            $k = $res->k;
