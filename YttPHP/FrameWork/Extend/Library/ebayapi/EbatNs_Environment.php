...
class EbatNs_Environment
{
    ...
    public function __construct($logLevel = 0, $configFile = 'sandbox')
    {
    ...
    public function init($logLevel = 0, $configFile)
    {
        ...
        //组合配置文件
        $configFile = dirname($_SERVER['SCRIPT_FILENAME']).'/Conf/data/ebay/'.$configFile.'.php';
		//echo $configFile.'<br>';exit;
    ...
    {
        if ($res->getAck() == AckCodeType::CodeType_Success || $res->getAck() == AckCodeType::CodeType_Warning)
        {
            return true;
        }
        else
