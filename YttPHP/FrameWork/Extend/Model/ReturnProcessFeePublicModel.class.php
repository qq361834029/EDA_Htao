<?php
class ReturnProcessFeePublicModel extends RelationCommonModel{
    
    protected $tableName    =   "return_process_fee";
    ...
    protected $_link        =   array(
        "detail"    =>  array(
            ...
            "foreign_key"   =>  "return_process_fee_id",
            "class_name"    =>  "ReturnProcessFeeDetail",
        )
    );
    
    protected $_validate    =   array(
        array("return_process_fee_no",'require','require',1),
        array("warehouse_id",'require','require',1),
		array("return_process_fee_no",'is_no',"valid_error",1), 
		array("return_process_fee_no",'','unique',1,'unique'),
		array("return_process_fee_name",'require','require',1),
		array("return_process_fee_name",'',"unique",1,'unique'),//验证唯一  
...
//        }
    ...
    public function indexSql(){
        $count          =   $this->count("id");
        $info['from']   =   ' return_process_fee ';
        $info['where']  =   getWhere($_POST);
        $info['field']  =   ' id,warehouse_id,return_process_fee_no,return_process_fee_name,shipping_type,comments ';
