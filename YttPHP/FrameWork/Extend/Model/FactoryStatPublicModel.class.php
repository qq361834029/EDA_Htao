 ...
 * @package  	Model
 * @author    	何剑波
 * @version 	2.1,2013-11-28
 */
class FactoryStatPublicModel extends AbsStatPublicModel { 
...
//                break;
	...
	public function afterHavePaidList($where){
		...
		if($_GET['show_type']<>'not_page'){
			$sql_total 	= 'select
								income_type,
								sum(original_money+if(object_type=202,discount_money,0)) as original_money,
								sum(discount_money) as discount_money,
								sum(should_paid+if(object_type=202,discount_money,0)) as should_paid,
								sum(have_paid) as have_paid,
								sum(need_paid) as need_paid
								from factory_paid_detail where '.$where.' group by income_type';
...
///				$ext_title = $temp[$value['is_close']]['object_type'] == 204 ? L('closeout') : L('checkout');				
	...
	public function getIndex() {
		...
		$sql_field =  'select basic_id,company.*,comp_id,currency_id,-sum('.$money.') as total_need_paid,warehouse_id,
							...
							from factory_paid_detail left join company on company.id=factory_paid_detail.comp_id			
							where  '.$where.' ';
...
//		$list = $this->db->query($sql);	
		...
		foreach ($list['list'] as $key => &$value) {
			$value['detail_url'] = 	U('/FactoryStat/view/type/1/comp_id/'.$value['comp_id'].
