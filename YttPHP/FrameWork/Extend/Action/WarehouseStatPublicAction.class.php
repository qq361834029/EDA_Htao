...
class WarehouseStatPublicAction extends RelationCommonAction {
	
		function _initialize(){
			addLang('stat');
			parent::_initialize();
            if(getUser('role_type')==C('WAREHOUSE_ROLE_TYPE')){
                $_POST['comp_id'] = getUser('company_id');
            }
		...
		public function index() { 
			...
			$this->assign('company_currency_count',C('logistics_currency_count'));
			///display
        }
			$this->displayIndex(); 
		}
    	...
    	public function view() {  
	       ...
	       ///获取当前Action名称
            ...
            $this->assign('w_name',SOnly('warehouse', empty($_GET['warehouse_id'])?$_GET['comp_id']:$_GET['warehouse_id'], 'w_name'));
        ...
        public function _after_view($temp_file) {
            ...
            $rand_post['query']['comp_id']          = $_POST['warehouse_id'];
            $rand_post['query']['pay_class_id']     = $_POST['pay_class_id'];
            $rand_post['query']['currency_id']      = $_POST['currency_id'];
            $rand_post['date']['from_paid_date']    = $_POST['from_paid_date'];
            $rand_post['date']['to_paid_date']      = $_POST['to_paid_date'];
            $rand_post['query']['currency_id']      = $_POST['currency_id'];
            $rand_post['query']['warehouse_id']     = $_POST['warehouse_id'];
