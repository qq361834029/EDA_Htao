 ...
 */
include_once(dirname(__FILE__).'/BCGBarcode.php');
include_once(dirname(__FILE__).'/BCGColor.php');
include_once(dirname(__FILE__).'/BCGDrawException.php');
include_once(dirname(__FILE__).'/drawer/BCGDrawJPG.php');
include_once(dirname(__FILE__).'/drawer/BCGDrawPNG.php');
...
class BCGDrawing {
     ...
     * @param int $h
     */
    public function drawMyBarcode($w,$h, $offsetY = 10){
        $size = $this->barcode->getDimension(0, 0);
        $w = $size[0]>$w?($size[0]+10):$w;
        $offsetX = ($w-$size[0])/2;
        $this->barcode->setOffsetX($offsetX);
        $this->barcode->setOffsetY($offsetY);
        $this->w = max(1, $w);
        $this->h = max(1, $h);
        $this->init();
        $this->barcode->draw($this->im);
