{wz print=false}
<form method="POST" action="{'FactoryStat/view'|U}"  beforeSubmit="checkSearchForm" validity="empty" id="search_form">
...
__SEARCH_START__
			<dl> 
				<dt>
					...
					<input type='text' url="{'/AutoComplete/expressName'|U}" value="{$factory_name}" jqac>__*__
				</dt>
				...
				<dt>
					...
					<input type="text" name="temp[pay_class_name]" value="{$pay_class_name}" url="{'/AutoComplete/fundsClassName'|U}" where="{" to_hide=1 and relation_object=3 and pay_type=1 "|urlencode}" jqac>
				</dt>					
...
{include file="FactoryStat/list_view.tpl"}
</div>
<script>
