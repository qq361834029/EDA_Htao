...
{assign var="deal_state" value=['return_sale_order_state'=>[1,2,3,4,9]]}
{/if}
...
{if $rs and $rs.is_related_sale_order != C('IS_RELATED_SALE_ORDER')}
	{assign var=thead value=[$lang.product_id,$lang.product_no,$lang.custom_barcode,$lang.product_name,$lang.return_quantity,$lang.treatment,$lang.warehouse_location]}
	{assign var="deal_state" value=['return_sale_order_state'=>[2,3,4,5,6,7,8,10]]}
{else}
	{assign var=thead value=[$lang.product_id,$lang.product_no,$lang.custom_barcode,$lang.product_name,'flow_quantity',$lang.return_quantity,$lang.treatment,$lang.warehouse_location]}
{/if}{$item.in_quantity}
{if $smarty.const.ACTION_NAME=='view' }
    {if $rs and $rs.is_related_sale_order != C('IS_RELATED_SALE_ORDER')}
 	{assign var=thead value=[$lang.product_id,$lang.product_no,$lang.custom_barcode,$lang.product_name,$lang.return_quantity,$lang.treatment,$lang.storage_quantity,$lang.warehouse_location]}
    {else}
        {assign var=thead value=[$lang.product_id,$lang.product_no,$lang.custom_barcode,$lang.product_name,'flow_quantity',$lang.return_quantity,$lang.treatment,$lang.storage_quantity,$lang.warehouse_location]}
...
<tr index="{$index}" class="{$none}">
	...
	{td id="span_product" class="t_left" viewstate=$deal_state span_product="{$item.product_id}" view="product_no" width="150"}
		...
		<input type="text" name="temp[{$index}][product_no]" id="product_no_show" url="{'AutoComplete/product'|U}" {if $rs.factory_id}where="{"factory_id={$rs.factory_id}"|urlencode}"{/if} {if $edit_sku}class="w150 disabled" readonly{else} jqac class="w150" {/if} value="{$item.product_no}">
	{/td}
	...
	{td id="span_quantity" span_quantity="{$item.edml_quantity}" viewstate=$deal_state type="flow_quantity" view="edml_quantity" tfoot=['total_col_qn'=>''] width="80" tfoot_value=$rs.detail_total.dml_quantity class="t_right"}
	    ...
	    <input type="hidden" name="storage[{$index}][return_sale_order_number]" value='{$item.edml_quantity}'>
    {/td}
	{td viewstate=$deal_state type="treatment" view="treatment" width="50" } 	  
		...
		{if $item.no_show!='1'}		   
		   {if $item.return_service} 		      
		       <a  id="icon"  title="{L('treatment')}" style="cursor:pointer" onclick="$.quicklyShowReturnService({$index}, this, '{$service_action}', '1','',{$rs.return_sale_order_state|default:0});"><font id='ft' color="red">{$item.return_service_no}</font></a>
		   {else}		      
		       <a id="icon" class="icon icon-help"   title="{L('treatment')}" style="cursor:pointer" onclick="$.quicklyShowReturnService({$index}, this, '{$service_action}', '1','',{$rs.return_sale_order_state|default:0});"><font id='ft' color="red"></font></a>	  
		   {/if}		 	  
		{/if}		
	{/td}
    {if $smarty.const.ACTION_NAME=='view'}
    {td class="t_right"}
        <input type="hidden" name="detail[{$index}][return_sale_order_number]" viewstate=$deal_state type="flow_quantity"  value='{$item.quantity}'>
        {$item.in_quantity}
	{/td}
    {/if}
    {td id="span_location_no" class="t_left" view="location_no" width="150" tfoot_id="total_barcode_no" tfoot_value="{$rs.detail_total.total_produc}"}
        <input type="hidden" name="detail[{$index}][location_id]" id="location_id" value="{$item.location_id}">            
        <input type="text" name="detail[{$index}][location_no]" value="{$item.location_no}" url="{'AutoComplete/returnLocationNo'|U}"{if {$item.warehouse_id}} where ="warehouse_id in ({$item.warehouse_id}_warehouse_id)"{else}where ="1" {/if}jqac {if $rs.return_sale_order_state == C('PROCESS_COMPLETE') && $login_user.role_type!=C('SELLER_ROLE_TYPE')}class="w150 disabled" readonly {else}class="w150" {/if}{$readonly}>
	{/td}  
	{detail_operation}
</tr>
{/detail_table}