{tr from=$list.list }
	...
	{/if}
	{td merge='product_id,color' class="$td_class"} 
		{if $item.color==1}
			{$item.class_name}
		{else}
			{$item.product_no}
		{/if} 
	{/td}
	{td merge='product_id,color' class="$td_class"} {if $item.color==0}{$item.product_name}{/if} {/td}
	{td merge='product_id,color' class="$td_class t_right"}
		{if $item.color==0}
