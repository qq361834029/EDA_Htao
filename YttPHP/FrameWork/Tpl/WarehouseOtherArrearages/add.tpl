...
<form action="{'WarehouseOtherArrearages/insert'|U}" method="POST" onsubmit="return false">
    	...
    	<tr>
    		<td colspan="4">
				...
				<ul> 
					...
					<li>
						{$lang.warehouse_name}：
						<input type="hidden" name="comp_id" id="comp_id" value="{$smarty.post.query.id}" onchange="factoryLimitFundsRelationDoc(4)" >
						<input type="text" name="comp_name" id='comp_name' url="{'AutoComplete/warehouse'|U}" jqac>__*__
					</li>
                    {else}
					...
					<li>
						...
						<input type="text" url="{'/AutoComplete/fundsClassName'|U}" where="{" to_hide=1 and relation_object=5 and pay_type=1 "|urlencode}" jqac>__*__ 
					</li>
					<li  style="display:none;">
						...
						<input type="text" name="reserve_id" id="reserve_id" url="{'/AutoComplete/fundsRelationDocNo'|U}" where="warehouse_id={if $login_user.role_type != C('WAREHOUSE_ROLE_TYPE')}0{else}{$login_user.company_id}{/if}" jqac><span id="reserve_require_span">__*__</span>
					</li>
