...
<td expand="1" width="30" align="center" id="expand"><a href="javascript:void(0)" onclick="$.showExpand('getSaleStorageByProducts','index_{$item.product_class_id}_{$key}','{$item.product_class_id}');"><span class="icon icon-pattern-plus"></span></a></td>
<td>{$item.class_name}</td>
{if 'storage_format'|C>=2}<td class="t_right">{$item.dml_quantity}</td>{/if}
<td class="t_right">{$item.dml_sale_storage}</td>
</tr>
{/foreach}
</tbody>
<tfoot>
<tr class="red">
   ...
   {if 'storage_format'|C>=2} <td class="t_right">{$list.total.dml_quantity}</td>{/if}
    <td class="t_right">{$list.total.dml_sale_storage}</td>
</tr>
</tfoot>
</table>
<script>
