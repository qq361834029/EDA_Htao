{wz print=false}
<form method="POST" action="{'WarehouseStat/view'|U}"  beforeSubmit="checkSearchForm" validity="empty" id="search_form">
...
__SEARCH_START__
			<dl>				
                {if $login_user.role_type != C('WAREHOUSE_ROLE_TYPE')}
                    ...
                    <input value="{$smarty.get.comp_id}" type="hidden" name="comp_id">
				...
				<dt>
					...
					<input type="text" name="temp[pay_class_name]" value="{$pay_class_name}" url="{'/AutoComplete/fundsClassName'|U}" where="{" to_hide=1 and relation_object=5 and pay_type=1 "|urlencode}" jqac>
				</dt>					
...
{include file="WarehouseStat/list_view.tpl"}
</div>
<script>
