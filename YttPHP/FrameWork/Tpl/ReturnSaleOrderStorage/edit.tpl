<form action="{'ReturnSaleOrderStorage/update'|U}" method="POST" name="ReturnSale_addReturnOrder">
					...
					<ul> 
						...
						{/if}	
                            ...
                            <li>
                                ...
                                {radio data=C('YES_NO') name="storage_abnormal" id="storage_abnormal" onclick="showStorageAbnormalReason(this)" value="`$rs.storage_abnormal`"}
                            </li>
                            ...
                            <li {if !$rs.storage_abnormal}style="display: none"{/if}>
                                ...
                                {select data=C('STORAGE_ABNORMAL_REASON') name="storage_abnormal_reason" id="storage_abnormal_reason" empty=true combobox="1" value="`$rs.storage_abnormal_reason`"}__*__
                            </li>
                            ...
                            <li {if in_array($smarty.const.LANG_SET,C('LONG_LANG_TPL'))}style="margin: 0 25px 0 75px;"{/if}>
                                ...
                                <input type="text" id="outer_pack_name" name="temp[package_name]" url="{'AutoComplete/packageNameUse'|U}" where="warehouse_id={$rs.warehouse_id}"  value="{$rs.outer_pack_name}" jqac />__*__
                                </div>
                            </li>                   
                            <li>
                                <div {if $rs.outer_pack!=2}style="display: none;"{/if}>
                                {$lang.outer_pack_quantity}：
                                <input type="text" id="outer_pack_quantity" name="outer_pack_quantity"  value="{$rs.outer_pack_quantity}" class="spc_input" />__*__
                                </div>
                            </li>
                            </div>
                            <div style="float: left">
                            ...
                            <li {if in_array($smarty.const.LANG_SET,C('LONG_LANG_TPL'))}style="margin: 0 25px 0 75px;"{/if}>
                                ...
                                <input type="text" id="within_pack_name" name="temp[package_name]" url="{'AutoComplete/packageNameUse'|U}" where="warehouse_id={$rs.warehouse_id}"  value="{$rs.within_pack_name}" jqac />__*__
                                </div>
                            </li> 
                            <li>
                                <div {if $rs.within_pack!=2}style="display: none;"{/if}>
                                {$lang.within_pack_quantity}：
                                <input type="text" id="within_pack_quantity" name="within_pack_quantity" value="{$rs.within_pack_quantity}" class="spc_input"/>__*__
                                </div>
                            </li> 
                            </div>
					</ul>
