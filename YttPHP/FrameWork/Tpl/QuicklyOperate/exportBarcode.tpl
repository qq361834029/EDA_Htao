...
{detail_table from=$rs action=['exportBarcode'] op_show=$op_show thead=[$lang.product_no,$lang.quantity] tfoot=false}
<tr index="{$index}" class="{$none}" class="t_left">
	{td type="product_id" id="span_id" class="t_left `$w_class`"}
		...
		<input itemto="dialog-export_barcode" type="text" name="temp[{$index}][no]" value="{$item.no}" url="{'AutoComplete/productNo'|U}" jqac style="width: 95%;" {$disabled}>
	{/td}
