<form action="{'OtherRevenue/insert'|U}" method="POST" onsubmit="return false">
...
<tbody> 
    ...
    <tr>
      <td>{$lang.income_class}：</td>
      <td class="t_left">
      <input type="hidden" name="pay_class_id" id="pay_class_id" value="{$smarty.post.query.pay_class_id}" >
            	<input type="text" name="oulay_name" id='oulay_name' url="{'AutoComplete/IncomeLay'|U}" jqac>__*__
            	{quicklyAdd module="PayClass" lang="add_income" type=1}
      </td>
    </tr>
     <tr>
      <td>{$lang.is_cost}：</td>
