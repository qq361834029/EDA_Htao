...
<input type="hidden" name="detail[{$index}][pre_delivery_detail_id]" value="{$item.pre_delivery_detail_id}">
	{td id="span_product"  view="product_no" width="90" class="t_left" viewaction=['add','edit','view'] } 
	...
	{/td} 
	{td class="t_right" type="flow_quantity" view="dml_quantity" width="46" tfoot=[total_quantity=>""] tfoot_value="{$rs.detail_total.dml_quantity}"}
	...
	{td type="flow_mantissa" view="dd_mantissa" width="40"}
		<input type="hidden" value="{$item.mantissa}" name="detail[{$index}][mantissa]">
	...
	{td view="dml_discount" width="6%" type="flow_delivery_discount" width="46"  class="t_right"}	
		<input type="text" name="detail[{$index}][discount]" class="w40" value="{$item.edml_discount}" row_total_disount>%
	{/td}
