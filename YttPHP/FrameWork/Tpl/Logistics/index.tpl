{wz}
<form method="POST" action="{'Logistics/index'|U}" id="search_form">
__SEARCH_START__
<input type="hidden" id="rand" name="rand" value="{$rand}">
<dl> 
	<dt>
		<label>{$lang.logistics_name}：</label>
		<input type="text" name="like[comp_name]" value="{$smarty.post.like.comp_name}" url="{'AutoComplete/logisticsName'|U}" jqac />
	</dt>
	<dt>
...
{include file="Logistics/list.tpl"}
</div> 