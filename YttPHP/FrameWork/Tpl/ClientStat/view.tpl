{wz print=false}
<form method="POST" action="{'ClientStat/view'|U}"  beforeSubmit="checkSearchForm" validity="empty" id="search_form">
...
__SEARCH_START__
			<dl> 
				...
				<dt>
					...
					<input type='text' url="{'/AutoComplete/factory'|U}" value="{$factory_name}" jqac>__*__
				</dt>
				{else}
					<input type="hidden" name="comp_id" value="{$login_user.company_id}">
				{/if}					
				...
				<dt>
					...
					<input type="text" name="temp[pay_class_name]" value="{$pay_class_name}" url="{'/AutoComplete/fundsClassName'|U}" where="{" to_hide=1 and relation_object=2 "|urlencode}" jqac>
				</dt>					
				...
				<dt>
					...
					<input type="text" id="date_from{$date_suffix}" name="date[from_paid_date]" value="{$smarty.get.from_paid_date}" class="Wdate spc_input_data" onClick="WdatePicker({ minDate:'#F{ $dp.$D(\'date_to{$date_suffix}\',{ M:-2,d:1 });}',maxDate:'#F{ $dp.$D(\'date_to{$date_suffix}\'); }'})"/>
					<label>{$lang.to_date}</label>
					<input type="text" id="date_to{$date_suffix}" name="date[to_paid_date]" value="{$smarty.get.to_paid_date}" class="Wdate spc_input_data" onClick="WdatePicker({ minDate:'#F{ $dp.$D(\'date_from{$date_suffix}\'); }',maxDate:'#F{ $dp.$D(\'date_from{$date_suffix}\',{ M:2,d:-1 }); }' })"/>
				</dt> 
			</dl> 
__SEARCH_END__
</form>
...
{include file="ClientStat/list_view.tpl"}
</div>
<script>
