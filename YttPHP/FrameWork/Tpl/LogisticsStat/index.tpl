{wz}
<form method="POST" action="{"LogisticsStat/{$smarty.const.ACTION_NAME}"|U}" id="search_form">
__SEARCH_START__
		<dl>
		   		<dt>
					<label >{$lang.logistics_name}：</label>
					<input type="hidden" name="query[comp_id]" value="{$smaty.get.query.comp_id}">
					<input type="text" name="logistics_name" url="{'/AutoComplete/logistics'|U}" jqac>
            	</dt>       
            	...
            	<dt>
					<label>{$lang.logistics_not_arrearage}：</label>
...
{include file="LogisticsStat/list.tpl"}
</div> 
