...
/// ------------
/// $Id: common.php 2929 2012-05-02 06:45:47Z liu21st@gmail.com $
 ...
 * @package  Common
 * @author   liu21st <liu21st@gmail.com>
 * @version  $Id: common.php 2929 2012-05-02 06:45:47Z liu21st@gmail.com $
...
function D($name='') {
    ...
    if(class_exists($class,true)) {
    	...
    	$class = basename($name.'PublicModel');
    	if(class_exists($class,true)) {
    		$model = new $class();
    	}else {
...
function T($name='') {
    ...
    import($name.'Status');
    $class   =   basename($name.'Status');
    if(class_exists($class,true)) {
        $model = new $class();
    }else {
    	$class = basename($name.'PublicStatus');
    	if(class_exists($class,true)) {
    		$model = new $class();
    	}
    }
...
function A($name) {
    ...
    if(class_exists($class,false)) {
    	...
    	$class = basename($name.'PublicAction');
    	if(class_exists($class,true)) {
    		$action = new $class();
	        $_action[$name]  =  $action;
	        return $action;
...
function addLang($module){
	if (strpos($module,',')) {$module = explode(',',$module);}
	if (is_array($module)) {
		foreach ($module as $value){
			/// 读取当前模块公共语言包
			if (is_file(THINK_PATH.'Lang/'.LANG_SET.'/'.strtolower($value).'.php')){
				L(include THINK_PATH.'Lang/'.LANG_SET.'/'.strtolower($value).'.php');
			}
			/// 读取当前模块自定义语言包
			if (is_file(LANG_PATH.LANG_SET.'/'.strtolower($value).'.php'))
			L(include LANG_PATH.LANG_SET.'/'.strtolower($value).'.php');
		}
	}else {
		/// 读取当前模块公共语言包
		if (is_file(THINK_PATH.'Lang/'.LANG_SET.'/'.strtolower($module).'.php')){
			L(include THINK_PATH.'Lang/'.LANG_SET.'/'.strtolower($module).'.php');
		}
		/// 读取当前模块自定义语言包
		if (is_file(LANG_PATH.LANG_SET.'/'.strtolower($module).'.php'))
		L(include LANG_PATH.LANG_SET.'/'.strtolower($module).'.php');
	}
...
/// 执行行为
function B($name, &$params=NULL,$brf_method=false) {
    G('behaviorStart');
    $class = $name.'Behavior';
    if(class_exists($class,true)) {
        $behavior = new $class();
    }else {
    	$class = $name.'PublicBehavior';
    	$behavior = new $class();
    }
    if ($brf_method===true) {
    	$behavior->brf($params);
