...
if(!IS_CLI) {
    ...
    if(!defined('_PHP_FILE_')) {
        if(IS_CGI) {
            ...
            define('_PHP_FILE_',rtrim($http_var.$_SERVER['HTTP_HOST'].$_SERVER['SCRIPT_NAME'],'/'));
			*/
			define('_PHP_FILE_',    rtrim($_SERVER['SCRIPT_NAME'],'/'));
        }
    }
    define('__APP_ROOT__',dirname(_PHP_FILE_));
    if(!defined('__ROOT__')) {
        /// 网站URL根目录
	...
	defined('FrameName') or define('FrameName', 'YttPHP');
	$local_host	= array(
		'192.168.0.202', '192.168.0.203', '192.168.0.254',
		'202.amoydream.com', '203.amoydream.com', '254.amoydream.com'
	);
	if(in_array($_SERVER['HTTP_HOST'], $local_host)){
		defined('PUBLIC_PATH') or define('PUBLIC_PATH', __ROOT__.'/' . FrameName . '/Public/');
	} else {
		defined('PUBLIC_PATH') or define('PUBLIC_PATH', __ROOT__.'/PublicCore/');
	}
	defined('APP_PUBLIC_PATH') or define('APP_PUBLIC_PATH', APP_PATH.'Public/');//项目公共目录 added by jp 20140304
	defined('APP_FONTS_PATH') or define('APP_FONTS_PATH', APP_PUBLIC_PATH.'Fonts/');//项目字体目录 added by jp 20140304
	defined('APP_EXCEL_PATH') or define('APP_EXCEL_PATH', APP_PUBLIC_PATH.'Excel');//项目Excel导入模板目录 added by jp 20140331
...
defined('LANG_PATH') or define('LANG_PATH', RUNTIME_PATH.'Lang/'); /// 项目语言包目录
defined('TMPL_PATH') or define('TMPL_PATH',APP_PATH.'Tpl/'); /// 项目模板目录
defined('TMPL_PATH2') or define('TMPL_PATH2',THINK_PATH.'Tpl/'); /// 项目模板目录
...
defined('BARCODE_PATH') or define('BARCODE_PATH',   RUNTIME_PATH.'Barcode/'); /// 项目条形码目录 added by jp 20140304
define('EXCEL_PATH', __ROOT__.'/'.APP_NAME.'/Runtime/Data/Excel');        /// 附件上传地址
define('WAY_BILL_PATH', DATA_PATH . 'WayBill/');        /// 快递面单保存目录
...
function load_runtime_file() {
    ...
    $list = array(
        ...
        CORE_PATH.'Core/JsonException.class.php',  /// 异常处理类
        CORE_PATH.'Core/Behavior.class.php',
        CORE_PATH.'Core/Status.class.php',
    );
    /// 加载模式文件列表
...
function check_runtime() {
    ...
    if(!is_dir(DATA_PATH.'/Excel/'))	mkdir(DATA_PATH.'/Excel/');	/// 导入模板生成目录
    if(!is_dir(DATA_PATH.'/Uploads/'))	mkdir(DATA_PATH.'/Uploads/');	/// 附件上传目录
	if(!is_dir(DATA_PATH.'/ApiXml/'))	mkdir(DATA_PATH.'/ApiXml/');	/// ApiXml目录
    if(!is_dir(LANG_PATH))	mkdir(LANG_PATH);	/// 语言包生成目录
    return true;
}
...
function build_app_dir() {
    ...
    if(is_writeable(APP_PATH)) {
        $dirs  = array(
            ...
            LIB_PATH.'Behavior/',
            LIB_PATH.'Widget/',
			BARCODE_PATH,//added by jp 20140304 条形码目录
            );
        foreach ($dirs as $dir){
